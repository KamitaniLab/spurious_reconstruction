...
def main():
    ...
    min_dist_list = [0.0, 0.01, 0.05, 0.1, 0.5, 1.0]
    n_neighbors_list = [5, 15, 30, 50, 100]
    for min_dist, n_neighbors in product(min_dist_list, n_neighbors_list):
        save_dir = os.path.join(save_base_dir, 'param_sweep_umap', network, feature_name)
        ...
        np.save(f'{save_dir}/nsd_embedding_norm_md_{min_dist}_nn_{n_neighbors}.npy', embeddings)
        print(f'Finish analysis saved at {save_dir}')


        if not os.path.isfile(f'{save_dir}/nsd_mean_md_{min_dist}_nn_{n_neighbors}.npy'):
            np.save(f'{save_dir}/nsd_mean_md_{min_dist}_nn_{n_neighbors}.npy', m)
        if not os.path.isfile(f'{save_dir}/nsd_norm_md_{min_dist}_nn_{n_neighbors}.npy'):
            np.save(f'{save_dir}/nsd_norm_md_{min_dist}_nn_{n_neighbors}.npy', s)

        # save umap 
        umap_name = f'nsd_umap_md_{min_dist}_nn_{n_neighbors}.pkl'
