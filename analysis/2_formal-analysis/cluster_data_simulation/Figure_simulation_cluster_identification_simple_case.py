...
class CIA_SimulationSettings:
    ...
    num_test:int = 50#int(num_train // num_inner_clusters)
...
def main(intra_inter_ratio):
    ...
    num_inner_clusters_list = np.array([3])
    num_inner_clusters = num_inner_clusters_list[0]
    print(f'dim: {dim}')
    # %%
    ...
    cia_factory.inner_cluster_centers = np.array([[0,0],
                                                        [10, 0],
                                                        [0, 10]])
    cia_factory.outer_cluster_centers = np.array([[12,12],
                                                        ])
    # %% 
    ...
    simulator = SimpleClusterSimulator(cia_factory, 
                            ...
                            num_oos = factory_simulator_settings.num_test,
                            num_oos_repetition =3)
    ...
    in_true = simulator.Y_test
    out_true = simulator.Y_oos
    
    in_predict = simulator.y_test_pred_dec
    out_predict = simulator.y_oos_pred_dec
    
    # %%
    fig = plt.figure(figsize=(6,6))
    ax = fig.add_subplot(111)
    ax.scatter(in_predict[:,0], in_predict[:,1], c=[137/255, 201/255, 151/255])
    ax.scatter(out_predict[:,0], out_predict[:,1], c="#FFCCB2", )
    
    ax.scatter(in_true[:,0], in_true[:,1], c=[26/255, 90/255, 46/255])
    ax.scatter(out_true[:,0], out_true[:,1], c=[(230 / 255, 88 / 255, 13 / 255)])
    ax.set_ylim(-5, 15)
    ax.set_xlim(-5, 15)
    # save 
    save_dir = f'./results/assets/fig09/'
    os.makedirs(save_dir, exist_ok=True)
    fig.savefig(f'{save_dir}/cluster_identifiction_simple.pdf')
