...
def plot_brain_and_latent_spaces(simulator, plot_settings, embed=True):
    ...
    ax.scatter(emb_Y_pred_dec[...,1], emb_Y_pred_dec[...,0], c='0.8',
    ...
    ax.scatter(emb_Y_train[..., 1], emb_Y_train[..., 0], c=[(26/255, 90/255, 46/255)], 
               s=plot_settings.size, alpha=plot_settings.alpha,
...
class ODC_SimulationSettings:
    ...
    short_cluster_var: float =0.000000001
    long_cluster_var: float = 0.1
...
if __name__ == "__main__":
    ...
    # %%
    # Plot the results
    fig = plot_brain_and_latent_spaces(odc_simulator,plot_settings, embed=True)
    print(np.max(np.abs(odc_simulator.X_oos), 0)/np.max(np.abs(odc_simulator.X_train), 0) )
    # %%
    ...
    # %%
    ## Increasing noise variance to hinder the cluster structure in the brain space
    # Generate dataset via factory settings
    odc_factory = ClusterProjectionDataFactory(
            input_dim=factory_simulator_settings.dim,
            output_dim=factory_simulator_settings.dim,
            num_inner_clusters=factory_simulator_settings.num_inner_clusters,
            num_outer_clusters= factory_simulator_settings.num_outer_clusters,
            noise_var=0.5, #factory_simulator_settings.noise_var,
            cluster_var= factory_simulator_settings.short_cluster_var,#/factory_simulator_settings.dim,
            inter_cluster_var= factory_simulator_settings.long_cluster_var,#/factory_simulator_settings.dim,
            seed=factory_simulator_settings.seed,
        )
    ...
    odc_factory = ClusterProjectionDataFactory(
            input_dim=factory_simulator_settings.dim,
            output_dim=factory_simulator_settings.dim,
            num_inner_clusters=3, # factory_simulator_settings.num_inner_clusters,
            num_outer_clusters= factory_simulator_settings.num_outer_clusters,
            noise_var= 0.5, #factory_simulator_settings.noise_var,
            cluster_var= factory_simulator_settings.short_cluster_var,#/factory_simulator_settings.dim,
            inter_cluster_var= factory_simulator_settings.long_cluster_var,#/factory_simulator_settings.dim,
    ...
    # %%
    ## Case where the output dimension collapse seems not to be observed
    odc_factory = ClusterProjectionDataFactory(
            input_dim=factory_simulator_settings.dim,
            output_dim=factory_simulator_settings.dim,
            num_inner_clusters=factory_simulator_settings.num_inner_clusters,
    ...
    # %%
    # Plot figure 6
    plot_settings_ = PlotSettings()
    plot_settings_.x_axis = 1
    plot_settings_.y_axis = 0


    # %%
    # Plot the results
    fig = plot_brain_and_latent_spaces(odc_simulator,plot_settings_, embed=True)
    print(np.max(np.abs(odc_simulator.X_oos), 0)/np.max(np.abs(odc_simulator.X_train), 0) )
    ...
    # %%
    ## Case where the output dimension collapse seems not to be occured and more nose 
    
    ## Case where the output dimension collapse seems not to be occured.
    odc_factory = ClusterProjectionDataFactory(
            input_dim=factory_simulator_settings.dim,
            output_dim=factory_simulator_settings.dim,
            num_inner_clusters=factory_simulator_settings.num_inner_clusters,
    ...
    odc_factory = ClusterProjectionDataFactory(
            input_dim=factory_simulator_settings.dim,
            output_dim=factory_simulator_settings.dim,
            num_inner_clusters=2, #factory_simulator_settings.num_inner_clusters,
            num_outer_clusters= factory_simulator_settings.num_outer_clusters,
            noise_var=0.5,
            cluster_var= 0.01,
            inter_cluster_var= 1.0,
