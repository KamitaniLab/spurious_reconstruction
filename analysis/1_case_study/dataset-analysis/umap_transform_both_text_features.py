...
def main():
    ...
    feat_dir_dict= {
        ...
        'god-train': f'/home/nu/data/contents_shared/ImageNetTraining/derivatives/features/{network}',
        'god-test': f'/home/nu/data/contents_shared/ImageNetTest/derivatives/features/{network}',
    ...
    np.save(f'{save_dir}/both_embedding_norm.npy', embeddings)
    print(f'Finish analysis saved at {save_dir}')


    if not os.path.isfile(f'{save_dir}/both_mean.npy'):
        np.save(f'{save_dir}/both_mean.npy', m)
    if not os.path.isfile(f'{save_dir}/both_norm.npy'):
        np.save(f'{save_dir}/both_norm.npy', s)

    # save umap 
    umap_name = 'both_umap.pkl'
